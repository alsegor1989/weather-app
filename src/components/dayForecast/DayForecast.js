...
const DayForecast = (props) => {
  ...
  const { process, setProcess, get5DaysForecast, getHistoricalWeather } =
    useWeatherService();
  ...
  const updateWeather = () => {
    ...
    if (props.day === "tomorrow") {
      ...
      getHistoricalWeather(selectedCity, new Date().setHours(0, 0, 0, 0) / 1000)
        ...
        .then((res) =>
          onHistoricalWeatherLoaded([...tempHistoricalWeather, ...res.hourly])
        )
  ...
  return (
    <>
      <Helmet>
        <meta name="description" content={`Forecast: ${props.day}`} />
